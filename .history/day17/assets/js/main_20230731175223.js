  ...
  for (let i = 1; i <= n; i++) {
    for (let j = 1; j <= n; j++) {
      if ((i % 2 === 0 && j % 2 !== 0) || (i % 2 !== 0 && j % 2 === 0)) {
        document.write(
          `<span style ="display: inline-block; width: 60px; height: 60px; border: 1px solid #000; background: #000"></span>`
        );
      } else {
        document.write(
          `<span style ="display: inline-block; width: 60px; height: 60px; border: 1px solid #000;"></span>`
        );
      }
    }
    document.write("<br/>");
...
function drawMultiplication() {
  for (let i = 1; i <= 10; i++) {
    for (let j = 1; j <= 10; j++) {
      document.write(
        `<span style ="display: inline-block; width: 100px; height: 100px; border: 1px solid #000;">${i} X ${j} = ${
          i * j
        }</span>`
      );
    }
