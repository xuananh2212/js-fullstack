...
function nestedArray(array, parentId, result) {
  for (var indexResult in result) {
    for (var indexArray in array) {
      if (array[index].parent === parentId) {
        nestedArray(array, array[index].id, result[index]);
      }
    }
    return result;
  }
...
console.log(nestedArray(array, 0, result));
