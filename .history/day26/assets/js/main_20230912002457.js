...
function checkWordsInLyric(lyricsSong) {
  ...
  var lyricsCurrent = lyricsSong.find((lyric) => {
    return (
      audio.currentTime >= lyric.words[0].startTime &&
      audio.currentTime <= lyric.words[lyric.words.length - 1].endTime
    );
  });
  if (lyricsCurrent) {
    var html = "";
    ...
    lyricsText && lyricsText.innerHTML = html;
  } else {
   lyricsText &&lyricsText.innerHTML = `${songs[index].nameSong} <br> Ca Sĩ: Sơn Tùng MTP`;
  }
}
