      ...
      btnAddOrder.addEventListener("click", function (e) {
        ...
        if (orders.length > 0) {
          ...
          if (order) {
            ...
            var productsDom = $(".tabele-order tbody tr");
            console.log(productsDom);
            productsDom.forEach(product => {
              if (product.id === productId) { 
                product.querySelector('input[type="number"]').textContent = order.quantity;
                product.querySelector("into-money") = order.quantity * product.price;
              }
            })
