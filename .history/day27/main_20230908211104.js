  ...
  function removeProduct(btnRemove) {
    ...
    modal.addEventListener("click", (e) => {
      if (e.target.matches(".agree")) {
        var productId = btnRemove.target.parentElement.parentElement.dataset.id;
        var index = orders.findIndex((order) => order.id === productId);
        orders.splice(index, 1);
        console.log(orders);
        totalmoney();
        btnRemove.target.parentElement.parentElement.remove();
