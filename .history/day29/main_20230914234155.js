...
const renderListTable = function () {
  topics.forEach((topic, indexTopic) => {
    var htmlActive = `<div class="list-items ative">
    ...
    lessions[indexTopic].topic.forEach((lession) => {
      var htmlNotActive = `<div class="list-items">
          <span class="text-value">Module:${indexLession++}:</span>
          <span class="text-content">${lession}</span>
        </div>`;
    });
  });
};
