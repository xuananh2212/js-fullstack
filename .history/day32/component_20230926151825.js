          ...
          [...templateNode.children].forEach((node) => {
            ...
            if (matchString) {
              node.addEventListener(matchString[1], function (e) {
                eval(matchString[2]);
                console.log(matchString[2]);
                var str1 = template;
                keys.forEach((key) => {
                  str1 = str1.replace(/{{.+?}}/, window[key]);
                });
                const templateElInner = document.createElement("template");
                templateElInner.innerHTML = str1;
                _this.innerHTML = "";
                _this.append(templateElInner.content.cloneNode(true));
              });
            }
          });
