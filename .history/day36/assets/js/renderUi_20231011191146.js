...
export let numberSteak = 0;
export let consecutive = 0;
export let arrAnswer = [];
...
export const audioIncorrect = $(".audio-incorrect");
export const audiocorrect = $(".audio-correct");
...
function HanldQuestions(e, numberCorrect) {
  ...
  const answerFind = arrAnswer.find((answer) => {
    console.log(answer.id, e.target.getAttribute("data-index"));
    return answer.id === e.target.getAttribute("data-index");
  });

  console.log(answerFind);
...
export function RenderHtml({ title, numberCorrect, answers }) {
  ...
  answers.forEach((answer) => {
    ...
    button.setAttribute("data-index", answer.id);
