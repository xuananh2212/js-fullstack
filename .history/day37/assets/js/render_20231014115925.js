...
async function handleSignout(tokens) {
  const { response } = await client.post("/auth/logout", tokens);
  if (response.ok) {
...
async function handleNewBlog(title, content, tokens, titleEL, contentEL) {
