...
async function handleSignout(token) {
  loadingEL.classList.remove("is-hidden");
  const { response } = await client.post("/auth/logout", {}, token);
  if (response.ok) {
...
async function getUser() {
  ...
  form.addEventListener("submit", function (e) {
    ...
    DateBlog.setMilliseconds(dateNow.getMilliseconds());
    console.log(dateNow, DateBlog);
    if (dateNow.getTime() === DateBlog.getTime()) {
      createdAt = "";
    } else {
