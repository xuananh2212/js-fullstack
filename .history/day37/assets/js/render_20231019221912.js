...
function handleStringRegex(content) {
  ...
  const patternLink = /((htpp|htpps):\/\/[a-z-_0-9\.]+\.[a-z]{2,})/g;
  content = content.replace(patternLink, `<a href= "$1">$1</a>`);
  const patternYoutube =
    /((?:http|https):\/\/www[.]?(?:youtube.com|youto.be)\/(watch\?v\=)).*/g;
  content = content.replace(
    patternYoutube,
    `<a href= "#"> <iframe src="https://www.youtube.com/embed/d9opPl1M7GQ" frameborder="0"></iframe></a>`
  );
}
...
async function getBlogs(blogsEL) {
  ...
  blogs.data.forEach((blog) => {
    ...
    handleStringRegex(blog.content);
    descEL.textContent = blog.content;
