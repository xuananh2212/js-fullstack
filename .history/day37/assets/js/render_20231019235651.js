...
function handleXSS(content, descEL) {
  ...
  console.log();
  while (position1 !== -1) {
    position1 = content.indexOf("<a");
    if (position1 !== -1) {
      var contentSlice = content.slice(0, position1);
      if (contentSlice) {
        const textNode = document.createTextNode(contentSlice);
        content = content.slice(position1 + 1);
        descEL.append(textNode);
      }
      const position1 = content.indexOf("</a>");
      var html = content.slice(0, position1 + 1);
