...
function handleXSS(content, descEL) {
  console.log(content);
  var positionContent = 0;
  console.log(positionContent);
  while (positionContent !== -1) {
    positionContent = content.indexOf("<a");
    console.log(positionContent);
    var contentSlice = content.slice(0, positionContent);
    if (contentSlice) {
      const textNode = document.createTextNode(contentSlice);
      content = content.slice(positionContent + 1);
      descEL.append(textNode);
    }
    const positionContent = content.indexOf("</a>");
    var html = content.slice(0, positionContent + 1);
