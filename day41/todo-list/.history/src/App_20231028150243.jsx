...
export class App extends Component {
     ...
     handleUpdateTodo = (value, id) => {
          const { todos } = this.state;
          if (/.{2,}/.test(value)) {
               var todoFind = todos.find(todo => todo._id === id);
               todoFind.todo = value;
               this.apiUpdateTodo(id, todoFind)
               this.handleStateUpdateTodos(todos, null);
               this.handleStateUpdateEditTodo(null);
               this.handleSateUpdateToast("sửa Thành công", 1);
               return;
          } else {
               this.handleSateUpdateToast("Todo cần ít nhất nhất là 2 kí tự", 2);
               return;
