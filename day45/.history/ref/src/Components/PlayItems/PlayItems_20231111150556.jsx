                    ...
                    playItems?.length > 0 && (
                         <>
                              ...
                              <div className="content">
                                   <span>Lần chơi thứ : {`${sumNumberPlay - numberPlay}/${sumNumberPlay}`}</span>
                                   <span>Số lần Nhập tối đa: {MAX_TIME}</span>
                                   <span >Tỷ lệ đúng:{correct.current === -1 ? `0%` : `${((MAX_TIME - (correct.current + 1)) * 100 / MAX_TIME).toFixed(2)}%`} </span>
                              </div>
                         </>
                    )
               }
