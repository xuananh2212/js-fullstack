...
export default function trelloList(state = initalState, action) {
     switch (action.type) {
          ...
          case "list/addTasks": {
               const listColumn = JSON.parse(JSON.stringify(state.listColumn));
               const itemColumn = listColumn.find(itemColumn => {
                    console.log(itemColumn, action.payload);
                    return itemColumn._id === action.payload._id
               });
               console.log(itemColumn);
               itemColumn.tasks.push(action.payload.newTask);
               return { ...state, listColumn }
          }
